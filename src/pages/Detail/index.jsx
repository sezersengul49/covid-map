...
const covidArr= Object.entries(data?.covid || {});
  return (
    ...
    <div className="min-h-[calc(100vh-74px)] bg-zinc-800 text-white place-items-center p-6 grid">
      <div className="min-h-[80vh] bg-white rounded-lg shadow-lg p-8 max-w-3xl max-md:w-full">
        ...
        <div className="flex gap-5 justify-between items-center mb-6">
           <Link to= "/" className="bg-gray-700 py-2 px-3 rounded-md hover:bg-gray-800">
           Geri</Link>

           <div className="flex items-center space-x-2">

        {isLoading ? (<Loader data-testid= "header-loader" type= "header" /> )
        :  (!error && 
        <>

          <img className="w-24 rounded-md" src= {data.country.flags.png} alt="" />
        ...
        {isLoading?  <Loader data-testid="card-loader"/> :error?  <Error  message={error}
        retry={()=> dispatch(getCountryData({code:countryCode,query}))}
        /> : 
